...
function s.initial_effect(c)
  ...
  --(3) Special Summon
  DAL.CreateTributeSummonListedMonsterEffect(c,CARD_DALSPIRIT_VIRUS)
...
s.listed_names={CARD_DAL_SPACEQUAKE,CARD_DALSPIRIT_VIRUS}
s.listed_series={SET_DAL}
--Pendulum Effects
--(1) Search
function s.thfilter1(c)
  return c:IsSetCard(SET_DAL) and c:IsTrap() and c:IsAbleToHand()
end
...
function s.desfilter(c)
  return c:IsCode(CARD_DALSPIRIT_VIRUS) and (c:IsLocation(LOCATION_HAND) or (c:IsFaceup() 
...
function s.hspconfilter(c)
  return c:IsSetCard(SET_DAL) and c:IsTrap() and not c:IsPublic()
end
