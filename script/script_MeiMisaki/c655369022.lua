...
function s.mentionMeiMisuki(c)
	return (c:IsCode(CARD_MEI_MISAKI) or (c:ListsCode(CARD_MEI_MISAKI) and c:IsSpellTrap())) and c:IsAbleToHand()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 then
		...
		if #g>0 then
			...
			if g:IsExists(s.mentionMeiMisuki,1,nil) then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
				local sg=g:FilterSelect(tp,s.mentionMeiMisuki,1,1,nil)
