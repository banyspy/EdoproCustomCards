...
function s.lvfilter(c)
	return c:IsFaceup() and c:IsMonster() and (c:HasLevel() or c:IsRankAbove(1))
end
function s.sumfilter(c)
	return c:IsSetCard(SET_TORINOKAMI) and c:IsSummonable(true,nil)
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.lvfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	--local hg=Duel.GetMatchingGroup(s.lvfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,0,nil)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if opt==0 then
		...
		local g=Duel.SelectMatchingCard(tp,s.lvfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
		if #g>0 then
			Duel.HintSelection(g,true)
			local lv
			if g:GetFirst():IsRankAbove(1) then
				lv=g:GetFirst():GetRank()
			else
				lv=g:GetFirst():GetLevel()
			end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_LVRANK)
			local lv=Duel.AnnounceLevel(tp,1,12,lv)
			local e1=Effect.CreateEffect(e:GetHandler())
			e1:SetType(EFFECT_TYPE_SINGLE)
