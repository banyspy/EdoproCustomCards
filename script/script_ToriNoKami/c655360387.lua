...
function s.initial_effect(c)
	...
	e2:SetProperty(EFFECT_FLAG_CANNOT_DIS_NEG_INA+EFFECT_FLAG_DAMAGE_STEP)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetRange(LOCATION_HAND)
	e2:SetHintTiming(0,TIMINGS_CHECK_MONSTER|TIMING_MAIN_END|TIMING_DAMAGE_STEP)
...
function s.atkdeffilter(c)
	return c:IsFaceup() and c:IsMonster() --and c:IsA
end
function s.sumfilter(c)
	return c:IsSetCard(SET_TORINOKAMI) and c:IsSummonable(true,nil)
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.atkdeffilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	local hg=Duel.GetMatchingGroup(s.atkdeffilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,0,nil)
	Duel.SetOperationInfo(0,CATEGORY_ATKCHANGE,hg,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DEFCHANGE,hg,1,0,0)
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if opt==0 then
		...
		local g=Duel.SelectMatchingCard(tp,s.atkdeffilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
		if #g>0 then
			Duel.HintSelection(g,true)
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELECT)
			local choice=Duel.SelectOption(tp,aux.Stringid(id,2),aux.Stringid(id,3))
			if choice==0 then
				local e1=Effect.CreateEffect(e:GetHandler())
				e1:SetType(EFFECT_TYPE_SINGLE)
				...
				local e2=e1:Clone()
				e2:SetCode(EFFECT_SET_DEFENSE_FINAL)
				e2:SetValue(g:GetFirst():GetDefense()*2)
				g:GetFirst():RegisterEffect(e2)
			end
