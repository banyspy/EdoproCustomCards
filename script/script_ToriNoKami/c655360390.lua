...
function s.rescon(fg)
	return function(sg,e,tp,mg)
		...
		return fg:IsSynchroSummonable(sg) and Duel.GetLocationCountFromEx(tp,tp,sg,fg)>0
	end
end
...
function s.summonfilter(c,tp,e)
	...
	(c:IsOriginalCode(655360388) or c:IsOriginalCode(655360389) or c:IsOriginalCode(655360390))
	and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,true) and not c:IsDisabled()
	and aux.SelectUnselectGroup(g,e,tp,1,#g,s.rescon(c),0)
end
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return --#g>1 --and s.summonfilter(c,tp,e) --and Duel.GetCurrentChain(true)==0--and 
		Duel.IsExistingMatchingCard(s.summonfilter,tp,LOCATION_EXTRA,0,1,nil,tp,e)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.AdjustInstantly(c)
	--Debug.Message(Duel.GetFlagEffect(tp,REGISTER_FLAG_TORINOKAMI))
	if not ToriNoKami.ToriNoKamiDontAskMoreThanOnce(tp,e,s.summonfilter) then return end

	local ag=Duel.GetMatchingGroup(s.thfilter,tp,LOCATION_MZONE,0,nil)
	if #ag>1 and Duel.IsExistingMatchingCard(s.summonfilter,tp,LOCATION_EXTRA,0,1,nil,tp,e) then

		local g=Group.CreateGroup() --Group of card that will be returned
		local tg -- Copy of Synchro that will be summoned
		--Duel.Hint(HINT_CARD,tp,id)
		repeat
		g:Clear()
		if Duel.SelectEffectYesNo(tp,c,aux.Stringid(id,0)) then
			tg = Duel.SelectMatchingCard(tp,s.summonfilter,tp,LOCATION_EXTRA,0,1,1,nil,tp,e):GetFirst()
			--Debug.Message(tg:GetCode())
			
			g=aux.SelectUnselectGroup(ag,e,tp,1,#ag,s.rescon(tg),1,tp,HINTMSG_RTOHAND,s.rescon2,nil,true)
		else
			g:DeleteGroup()
			return
		end
		until(#g>0 and tg:IsSynchroSummonable(g) and Duel.GetLocationCountFromEx(tp,tp,g,tg)>0)
		tg:SetMaterial(g)
		...
		ToriNoKami.ResetToriNoKamiFlag(tp)
		g:DeleteGroup()
	end
end
...
function s.thrmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return --Duel.IsExistingMatchingCard(s.thrmfilter,tp,LOCATION_DECK+LOCATION_GRAVE+LOCATION_REMOVED,0,1,nil) 
		true end
...
function s.thrmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if opt==0 then
		...
		local sc=Duel.SelectMatchingCard(tp,aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil):GetFirst()
		if sc then
