...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Add 1 "Xros" monster from your Deck to your hand
	...
	c:RegisterEffect(e5)
    --Shuffle "Xros" Link to Extra and add this card to hand
...
function s.thfilter(c)
	return c:IsSetCard(SET_XROS) and c:IsMonster() and c:IsAbleToHand() and not c:IsCode(id)
end
...
function s.dstg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and s.dsfilter(chkc) end
...
function s.tdfilter(c)
	return c:IsSetCard(SET_XROS) and c:IsMonster() and c:IsType(TYPE_LINK) and c:IsAbleToExtra()
end
