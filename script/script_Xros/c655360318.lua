...
function s.initial_effect(c)
	...
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_XROS),2,2,nil)
...
function s.thfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_XROS) and c:IsAbleToHand()
end
...
function s.tefilter(c)
	return c:IsSetCard(SET_XROS) and c:IsType(TYPE_PENDULUM) and not c:IsForbidden()
end
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetTarget(function(_,c) return not c:IsSetCard({SET_XROS}) and c:IsLocation(LOCATION_EXTRA) end)
	e1:SetReset(RESET_PHASE|PHASE_END)
	Duel.RegisterEffect(e1,tp)
	--Clock Lizard check
	aux.addTempLizardCheck(c,tp,function(_,c) return not c:IsOriginalSetCard({SET_XROS}) end)

	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_EXTRA|LOCATION_GRAVE|LOCATION_REMOVED,0,1,2,nil)
